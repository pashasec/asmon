...
def _render_diff_text(diff: SurfaceDiff) -> str:
    ...
    # Show critical/high CVEs first
    critical_cves = [c for c in diff.changes if c.entity_type == "cve" and c.severity == "critical"]
    ...
    for change_type, colour, label in [
        ...
        group = [c for c in diff.changes if c.change_type == change_type and c.entity_type != "cve"]
        if not group:
            continue
    ...
    # Show other CVEs (medium/low/info) at the end
    other_cves = [c for c in diff.changes if c.entity_type == "cve" and c.severity not in ["critical", "high"]]
    if other_cves:
...
def render_snapshot_summary(snapshot: Snapshot) -> str:
    ...
    # Active scan info
    if snapshot.active_scan_enabled:
        total_cves = sum(len(h.cves) for h in snapshot.hosts)
        critical_cves = sum(1 for h in snapshot.hosts for cve in h.cves if cve.severity == "critical")
        high_cves = sum(1 for h in snapshot.hosts for cve in h.cves if cve.severity == "high")
