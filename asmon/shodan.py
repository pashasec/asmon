...
class ShodanClient:
    ...
    def host_details(self, ip: str) -> HostRecord | None:
        """Fetch full details for a single IP. Returns None on any error."""
        try:
    ...
    try:
        ...
        for port_info in raw.get("data", []):
            try:
                ...
                logger.debug("Failed to parse service on port %s: %s",
                            port_info.get("port"), exc)
                continue
        ...
        return HostRecord(
            ...
            org=raw.get("org"),
            asn=raw.get("asn"),  # Now accepts str or int
        ...
        # Validation or parsing failed - return minimal record
